 ...
 *
 * NOTE: This file differs from the one fwadm(1M) uses in smartos-live
 * since firewaller ships on older platforms that don't support flock(3C).
 * In the future, once the minimum platform has been raised past 20150219,
 * firewaller can bump its lockfd dependency past 1.1.0 and use the same
 * locker.js file that fwadm(1M) uses.
 */
...
function acquireLock(callback) {
    tryOpen(function (err, fd) {
        ...
        mod_lockfd.lockfd(fd, function (err2) {
            if (err2) {
                releaseLock(fd);
                callback(err2);
                return;
