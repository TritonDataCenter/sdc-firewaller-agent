...
function populateRemoteVMs(opts, callback) {
    ...
    mod_vm.list(filter, function (err, vms) {
        ...
        fw.vms({ vms: vms, rule: opts.payload }, function (err2, matching) {
            if (err2) {
            ...
            fw.remoteTargets(payload, function (err3, targets) {
                ...
                opts.log.debug({ rule: opts.payload, targets: targets },
                    'Remote targets');
                ...
                var missing = opts.cache.missing(
                    opts.payload.owner_uuid, targets);
                if (prim.isEmpty(missing)) {
                    opts.log.debug({ rule: opts.payload },
                        'No missing targets: not fetching from VMAPI');
                    return callback(null, payload);
                }

                opts.log.debug({ rule: opts.payload, missing: missing },
